...
class Login extends CI_Controller {
	...
	public function registrasi()
    {
        ...
        if ($this->form_validation->run() == false) {
			$this->session->set_flashdata('gagal', 'Masukan Nama Petugas, Username dan Password');
            $this->load->view('registrasi');
        } else {
            $this->m_login->registrasi();
            $this->session->set_flashdata('pesan', 'Berhasil Registrasi Akun');
            redirect('login');
        }
    }
}
